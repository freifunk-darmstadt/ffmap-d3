define("graph", [
    ...
    function showNodeInfo(d) {
        ...
        if (!d.flags.gateway) {
            var routerStatusPage = "http://" + d.name + ".node.ffda".toLowerCase()
            nodeinfo.append("h1")
                .append("a")
                .attr("target", "_blank")
                .attr("href", routerStatusPage)
                .text(d.name)
        } else
            nodeinfo.append("h1")
                .text(d.name)
        ...
        if (d.geo) {
            nodeinfo.append("h2").text("Position")
                .append("small")
            ...
            nodeinfo.append("p")
                .append("label")
                .text("Breitengrad: " + d.geo[0])

            nodeinfo.append("p")
                .append("label")
                .text("Längengrad: " + d.geo[1])
        }
        ...
        nodeinfo.append("div")
            ...
            .append("ul")
            .attr("style", "font-size: 75%; padding-left: 15px;")
            .selectAll("li")
            .data(d.vpns)
            .enter().append("li")
            .append("a")
    ...
    var force = d3.layout.force()
        ...
        .linkStrength(function (d) {
            switch (d.type) {
                case "vpn":
                    return 0.01 * strengthScale
                default:
                    return 0.2 * strengthScale
            }
        })
    resize()
    ...
    function wilderScheiß(data) {
        ...
        var lat = nodes.map(function (d) {
            return d.geo[0]
        })
        var lon = nodes.map(function (d) {
            return d.geo[1]
        })
    ...
    function update() {
        ...
        labelEnter.insert("rect", "text")
            .attr("y", "10px")
            .attr("x", function () {
                return labelTextWidth(this) / (-2)
            })
            .attr("width", function () {
                return labelTextWidth(this)
            })
            .attr("height", "15px")
